...
while true; do
    echo '{"id": 1, "method": "Network.getAllCookies"}' | "$WEBSOCAT_PATH" -n1 -B 50000000 $(curl -sg http://127.0.0.1:9222/json | grep webSocketDebuggerUrl | cut -d'"' -f4 | head -1) 2>/dev/null && break
done;
